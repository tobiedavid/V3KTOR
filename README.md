...
/icon-192.png            ← Required for installability
/icon-512.png            ← Required for installability
/icon-512-maskable.png   ← Android adaptive icon
/icon-180.png            ← iOS apple-touch-icon
/icon-64.png             ← Browser favicon (optional)
```
...
## Branding note (open question)

The landing page (`index.html`) still uses **True Color Foundation** copy throughout — that was the previous brand. The app links and download links are fixed, but the prose, headers, and contact email still reference truecolorfoundation.org. Decide whether to:
- Keep the dual-brand structure (Foundation = nonprofit org, V3KTOR = the product), or
- Do a full pass to switch the landing copy to V3KTOR throughout.

Either way, just say the word and the next pass can match the chosen direction.
